...
-- 21. Saturday Club Session Attendance Tracker
...
-- 23. Resources Approval Flow Column
ALTER TABLE public.resources 
ADD COLUMN IF NOT EXISTS status text NOT NULL DEFAULT 'approved';

-- 24. Notice Board Enhancements (Pinned, Priority, Reactions, Comments)
...
CREATE POLICY nc_delete ON public.notice_comments FOR DELETE TO authenticated USING (user_id = auth.uid() OR public.has_role(auth.uid(), 'admin'::app_role));
